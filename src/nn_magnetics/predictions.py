...
def B(
    ...
    model = network_type.load_from_path(model_path)
...
def test_B_demag(position, dimension, polarization, susceptibility, X):
    magnet = mp.magnet.Cuboid(
        dimension=dimension,
...
if __name__ == "__main__":
    dimension = np.array((1, 2, 0.5))
    polarization = np.array((1, 1, 2))
    position = np.array((0, 0, 0))
    susceptibility = np.array((0.1, 0.2, 0.3))
    X = np.array([[1.4, 1.1, 3.5], [-1.4, 1.1, -1.5]])
