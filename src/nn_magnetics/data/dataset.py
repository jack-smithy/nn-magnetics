...
class AnisotropicData(Dataset):
    def __init__(
        self, path: Path | str, device: str = "cpu", dtype=torch.float64
    ) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> t.Tuple[torch.Tensor, torch.Tensor]:
        return (
            torch.tensor(self.X[index], device=self.device, dtype=self.dtype),
            torch.tensor(self.B[index], device=self.device, dtype=self.dtype),
        )
    ...
    def get_magnets(self) -> t.Tuple[torch.Tensor, torch.Tensor]:
        ...
        with ThreadPoolExecutor() as e:
            ...
            for future in as_completed(futures):
                _X, _B = future.result()
                X = torch.cat((X, torch.tensor(_X).unsqueeze(0))).to(self.dtype)
                B = torch.cat((B, torch.tensor(_B).unsqueeze(0))).to(self.dtype)
        return (
            X.to(self.device),
            B.to(self.device),
        )
